 ...
 *  time against 64% for everyone else (`sandbox/house-edge/check-positional-bias.ts`).
 *
...
export function tick(round: ERRound, steps: number): void {
  for (let s = 0; s < steps; s++) {
    ...
    // TERMINATION: a defender already down to dust is finished off rather than chased to an
    // asymptote. Keys on the DEFENDER's ring alone.
    if (D.hp <= DUST) dmg = D.hp;
    // A blow too small to register moves nothing — and it is NOT a kill. These two used to share a
    // branch (`D.hp <= DUST || dmg === 0n`), which was safe only while `basis` was the defender's
    // ring. With `basis` reading the attacker, `dmg === 0n` also means "the ATTACKER is spent", and
    // the old form paid that spent attacker the defender's whole ring.
    if (dmg === 0n) continue;
 ...
 *  is `enter` asserted against a known gross stake — see `tests/er-sim.test.ts`, and
