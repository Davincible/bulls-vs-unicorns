...
/// Hard ceiling on fighters in one round — re-exported from `crates/arena-state`, which is where it
/// now lives and where its derivation lives with it (why 48 and not 52, what binds the cap and what
/// turns out not to). Read that before moving the number.
///
/// IT WENT WITH `Round`, NOT ON ITS OWN MERITS. `Round.fighters` is `[Fighter; MAX_FIGHTERS]`, so the
/// constant is the length of the array that is 3,072 of the account's 3,248 bytes — it is layout, and
/// a layout crate that could not state its own array bound would not be one. ARENA-VAULT.md §8.1's
/// "anything those need in order to compile", and nothing beyond it: the fight-length table in its
/// doc comment travelled because it is the ARGUMENT for the number, not because the vault will ever
/// simulate a fight.
///
/// RE-EXPORTED RATHER THAN LEFT FOR CALLERS TO FIND, and that is not politeness. `tests/account.rs`
/// and `tests/compute.rs` spell it `bulls_arena::MAX_FIGHTERS` and are right to — at the point of use
/// it is a fact about THIS program's rounds. Re-exporting is the difference between extracting a
/// layout and breaking an API.
pub use arena_state::MAX_FIGHTERS;
...
// THE FIGHTER AND THE PHASE MACHINE — `crates/arena-state`, unchanged, re-exported.
//
// They moved for `Round`'s sake and not their own: `Fighter` is 64 of the account's bytes forty-eight
// times over, and `Phase` is the meaning of `Round.phase`. A vault that could read a round but could
// not tell `Settled` from `Lobby` would have to name the numbers itself, which is the drift
// ARENA-VAULT.md §3.4 is about, one field down.
//
// `Fighter`'s `Debug`/`PartialEq` are now behind `arena-state`'s `test-support` feature rather than
// `cfg(test)`, because `cfg(test)` cannot reach across a crate boundary. This program asks for them
// in `[dev-dependencies]`; see both Cargo.tomls. Nothing about the deployed bytes changed.
pub use arena_state::{Fighter, Phase};
...
/// THE ROUND, AS *THIS PROGRAM'S* ACCOUNT. One `repr(C)` field wide, and that field is the layout —
/// `arena_state::RoundState`, where every one of the eighteen fields and every word of their reasoning now
/// lives.
///
/// WHY THE STRUCT IS SPLIT IN TWO WHEN IT USED TO BE ONE, WHICH IS THE ONLY SURPRISING LINE IN THIS
/// FILE. ARENA-VAULT.md §3.4 needs `arena-vault` to check a settled round's solvency with
/// `Round::conserves()` — this program's definition, not a second one — and to decode the round's
/// bytes with this program's struct, not a copy. That makes `Round` a SHARED type, and a shared type
/// cannot carry `#[account(zero_copy)]`, because that attribute generates
/// `impl Owner for Round { crate::ID }` and `crate::ID` is a deployment, not a layout. The shared
/// crate must never learn a program id: `bulls-arena` has burned seven of them to the ER's bytecode
/// cache (see this file's header) and `arena-vault` is the program that must never churn.
///
/// AND THE `Owner` IMPL CANNOT SIMPLY BE WRITTEN HERE INSTEAD. It was tried, in the real dependency
/// graph, and rustc refuses:
///
/// ```text
...
/// ```
///
/// `Owner` takes no type parameters, so there is no local type anywhere in the impl for coherence to
/// hang it on. There is no attribute, no marker and no generic that gets round this. The compiler's
/// own suggestion is the wrapper, and the wrapper happens to say the true thing: **the layout is
/// shared; the ownership is one program's opinion about it.** `arena-vault` will hold the same
/// `arena_state::RoundState` with no `Owner` at all, because it reads rounds it does not own and checks
/// `round.owner == vault_arena.game_program` by hand (§3.4 step 0).
///
/// AND THE TWO TYPES MAY NOT SHARE A NAME, WHICH IS THE SECOND REASON THE SHARED ONE IS CALLED
/// `RoundState`. It was called `Round` first, and `anchor build` exits 1:
///
/// ```text
...
/// ```
///
/// Anchor's IDL builder disambiguates same-named types by full path and then refuses any account
/// name containing `::` (`anchor-lang-idl-0.1.2/src/build.rs`, `verify`). THE WRAPPER IS THE ONE THAT
/// HAD TO KEEP THE NAME, for the reason in the next paragraph, so the layout took the new one.
///
/// The visible cost is that `anchor build`'s own IDL now describes `Round` as a single field of type
/// `RoundState` rather than eighteen flat ones. Nothing in this repo consumes that file — the three
/// served and committed artefacts are `scripts/idlgen.py`'s output, and `idlgen --verify` still
/// reports `Round layout: 18 fields, 3248 B on chain` — and borsh cannot tell the two shapes apart
/// anyway, since a `repr(C)` struct of one field serialises as that field. Written down rather than
/// left in `target/idl/` for somebody to find.
///
/// WHAT DID NOT CHANGE, WHICH IS EVERYTHING THAT REACHES THE CHAIN. `#[repr(C)]` with one field puts
/// that field at offset 0, so the account's bytes are the layout's bytes and `size_of` is unmoved at
/// 3,240. The wrapper is *named* `Round`, so anchor's discriminator — `sha256("account:Round")[..8]`,
/// derived from the ident and nothing else — is the same eight bytes the deployed program and every
/// client already use. `the_wrapper_is_byte_for_byte_the_layout_it_wraps` asserts all four of those
/// claims rather than leaving them to be believed, and it is the test to read before touching this
/// struct. The offsets themselves are pinned once, in `arena-state`, by the body
/// `the_account_layout_is_exactly_what_the_clients_decode` calls — one set of assertions, called from
/// there and from here, so neither the definition nor the coverage is duplicated.
///
/// CALLERS DO NOT SEE THE SPLIT, WITH ONE EXCEPTION WORTH KNOWING BEFORE IT SURPRISES SOMEBODY.
/// `Deref`/`DerefMut` mean `r.pot`, `r.fighters[i]` and `r.conserves()` read exactly as they did; a
/// field access autoderefs through the wrapper and through the `RefMut` an `AccountLoader` hands out,
/// and every instruction body in this file compiled across the move untouched. What autoderef does
/// NOT preserve is DISJOINT FIELD BORROWS: `&mut r.fighters` together with `&r.seed` is two borrows
/// of two fields to the borrow checker, but two `deref` calls that each borrow the whole `Round`
/// once they go through this wrapper. Exactly one site in this file did that
/// (`a_bounded_catch_up_always_advances_and_says_when_it_is_short`, which now reads the seed into a
/// local first) and the fix is always the same: copy the field out before taking the mutable borrow.
/// The only other places that name the inner type are this declaration and the test constructors,
/// which have to build one.
#[account(zero_copy)]
pub struct Round {
    /// The layout. `pub` because tests construct a round directly (`fresh_round`, `fight_round`) and
    /// there is no reason for them to go through a constructor that would only re-state this line.
    pub state: arena_state::RoundState,
}

impl Round {
    /// The allocation `#[account(init, space = ...)]` makes, forwarded from the layout so that the
    /// number has one source. 3,248 B: see `arena_state::RoundState::SIZE` for the arithmetic and for
    /// what the rent costs.
    ///
    /// FORWARDED RATHER THAN INHERITED, BECAUSE ASSOCIATED CONSTS DO NOT AUTODEREF. Everything else
    /// about the layout reaches call sites through `Deref`; `Round::SIZE` would not, and without this
    /// line `space = Round::SIZE` would simply not compile. `the_wrapper_is_byte_for_byte_the_layout_it_wraps`
    /// asserts the two are equal so the forwarding cannot rot into a second number.
    pub const SIZE: usize = arena_state::RoundState::SIZE;
}

// The wrapper's whole ergonomic job, and deliberately nothing more: no `Deref` to a `&mut` behind a
// shared reference, no `From`/`Into`, no re-exported constructors. A wrapper that grows an API is a
// wrapper that has started to be a second definition.
impl core::ops::Deref for Round {
    type Target = arena_state::RoundState;
    fn deref(&self) -> &Self::Target { &self.state }
}
impl core::ops::DerefMut for Round {
...
mod lobby_tests {
    ...
    /// EVERY FIELD'S BYTE OFFSET, PINNED — the same assertions this test has always run, now with
    /// their body one crate away in `arena-state` and called from both sides.
    ///
    /// THE NAME AND THE COVERAGE ARE DELIBERATELY UNCHANGED, and that is the whole point of the
    /// shape. `Round`'s layout moved to `crates/arena-state` so that `arena-vault` can decode a round
    /// with this program's struct instead of a copy (ARENA-VAULT.md §3.4), and the offsets test had
    /// to go with it — "the offsets test is the thing that must have exactly one home". But
    /// `cfg(test)` is true only while compiling the crate under test, so a `#[test]` left over there
    /// would not run under `cargo test --manifest-path programs/bulls-arena/Cargo.toml` (the command
    /// this file's own IDL comment gives) nor under `mutation-test.sh`. One home for the assertions
    /// and no coverage here would have been the worse half of the trade.
    ///
    /// So the body is a `pub fn` behind `arena-state`'s `test-support` feature, which this package
    /// turns on in `[dev-dependencies]` and `cargo build-sbf` therefore never sees. Read the
    /// assertions and the reasoning there; there is exactly one copy of both. What is asserted HERE,
    /// about the wrapper that holds them, is `the_wrapper_is_byte_for_byte_the_layout_it_wraps`.
    #[test]
    fn the_account_layout_is_exactly_what_the_clients_decode() {
        arena_state::assert_the_account_layout_is_exactly_what_the_clients_decode();
    }

    /// THE WRAPPER IS THE THING IT WRAPS — the four claims `Round`'s doc comment makes about the move
    /// into `crates/arena-state`, asserted rather than argued.
    ///
    /// WHY THIS TEST HAS TO EXIST AT ALL. `the_account_layout_is_exactly_what_the_clients_decode`
    /// pins every byte offset of `arena_state::RoundState`, and it is the single most consequential test in
    /// this repo — but it now runs one crate away, against a struct that is not literally the one
    /// `#[account(init, space = Round::SIZE)]` allocates. These four lines are the bridge. Without
    /// them "the offsets test has exactly one home" (ARENA-VAULT.md §3.4) would be a claim about a
    /// type this program does not use, and the gap would be exactly wide enough to hide a second
    /// field added to the wrapper.
    ///
    /// AND THE DISCRIMINATOR IS THE ONE THAT COULD HAVE GONE WRONG SILENTLY. Anchor derives it from
    /// the type's IDENT — `sha256("account:Round")[..8]` — so the wrapper being named `Round` is
    /// load-bearing, not cosmetic. Had it been called `RoundAccount`, every artefact in
    /// `programs/bulls-arena/idl/` and the copy the live page fetches would describe an account the
    /// program no longer writes, and the failure would be a browser decoding garbage rather than a
    /// build refusing. Re-derived here from the string rather than copied from a table, so the
    /// assertion cannot agree with a mistake.
    #[test]
    fn the_wrapper_is_byte_for_byte_the_layout_it_wraps() {
        use core::mem::{align_of, offset_of, size_of};

        assert_eq!(offset_of!(Round, state), 0, "the layout must start where the account does");
        assert_eq!(
            size_of::<Round>(), size_of::<arena_state::RoundState>(),
            "the wrapper has grown a second field — the account is no longer the layout",
        );
        assert_eq!(align_of::<Round>(), align_of::<arena_state::RoundState>());
        assert_eq!(Round::SIZE, arena_state::RoundState::SIZE);

        assert_eq!(
            Round::DISCRIMINATOR,
            &solana_sha256_hasher::hashv(&[b"account:Round"]).to_bytes()[..8],
            "the account discriminator moved — every deployed round and every client that decodes \
             one is now unreadable by this program, and vice versa",
        );
    }
...
mod house_tests {
    ...
    ///
    /// The `arena_state::RoundState { .. }` is the one place this module has to name the inner type — see
    /// `Round`'s doc comment for why the account is a one-field wrapper. Every assertion below still
    /// reads `r.pot`, `r.conserves()` and so on, because field access and method calls autoderef
    /// through it.
    fn fresh_round() -> Round {
        Round { state: arena_state::RoundState {
            arena: Pubkey::default(),
            round_no: 1,
    ...
    fn a_bounded_catch_up_always_advances_and_says_when_it_is_short() {
        ...
        // The seed is read out FIRST rather than inline, and that is the one place the `Round`
        // wrapper is visible to a caller. `&mut one_shot.fighters` and `&one_shot.seed` used to be
        // borrows of two disjoint FIELDS, which the borrow checker allows; through `Deref` they are
        // two calls that each borrow the whole `Round`, which it does not. Hoisting the copy is the
        // whole fix — `seed` is `[u8; 32]`, `Copy`, and read before anything mutates — and it says
        // more plainly than the old `.clone()` did that the fight runs against the seed the round
        // already had. See `Round`'s doc comment for why the split exists at all.
        let seed = one_shot.seed;
        advance_fight(&mut one_shot.fighters, n, &seed, 0, final_cursor(n));
