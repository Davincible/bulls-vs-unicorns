...
//
// ============================================================================================
// THIS IS A SPIKE, NOT THE GAME. Phase 0 of the er-demo/ plan (the approved plan this session
// worked from is named "snug-floating-mitten"; see er-demo/scripts/spike-session-er.mjs for the
// script that exercises this program). programs/bulls-arena is proven, security-reviewed, and
// deployed at
// F59NksP2bYZhP4wD7fgR1sP729UHNPitrBiYrrKF1sYW — this is a COPY of it under a fresh devnet
// keypair (EJ8dAm3HnBY9UL4mYBUyDzLTvgkDGAp8cT3e2mgaLxQP), existing to answer exactly one
// question before any UI gets built on the assumption of an answer: does a MagicBlock session
// key actually authorize an instruction against an account that is delegated to an Ephemeral
// Rollup? Nobody — not MagicBlock's own examples, not any project — had verified that
// combination before this spike. Every part of this file except `Enter`/`enter()` is byte-for-
// byte identical to the proven program; do not treat this as a second source of truth for the
// game's design, and do not build on top of this program id.
// ============================================================================================
//
...
// Phase 0 spike only — session-keys 3.1.1, integrated into `Enter`/`enter()` alone. Syntax
// verified against the crate's own gpl_session example program (github.com/magicblock-labs/
// session-keys, programs/gpl_session/src/lib.rs) and docs.magicblock.gg's "Integrating Sessions
// In Your Program" page, not guessed — a prior research pass this session found the right pages
// but never compiled against them; this is the first real compile.
use session_keys::{session_auth_or, Session, SessionError, SessionToken};

declare_id!("EJ8dAm3HnBY9UL4mYBUyDzLTvgkDGAp8cT3e2mgaLxQP"); // SPIKE keypair: .devnet/session-spike-keypair.json — NEVER the proven program's id
...
pub mod bulls_arena {
    ...
    ///
    /// PHASE 0 SPIKE: `#[session_auth_or]` (session-keys 3.1.1) is the ONLY behavioural change
    /// versus the proven program. It runs BEFORE the body below: if `session_token` is present
    /// and valid (a real PDA, unexpired, bound to this program as `target_program` and to
    /// `player` as its `authority`), the transaction may be signed by the session key instead of
    /// `player`'s own wallet. If no session token is supplied, it falls back to requiring
    /// `signer.key() == player.key()` — ordinary direct-wallet signing, unchanged from before.
    /// Either way `who = ctx.accounts.player.key()` below is what actually gets credited; the
    /// session key/signer is never itself the fighter identity.
    #[session_auth_or(
...
/// PHASE 0 SPIKE. `player` and `signer` split apart on purpose — this is the whole shape session
/// keys forces: `player` is WHO gets credited (the fighter identity written into `Round.fighters`,
/// and the pubkey the SessionToken's `authority` must equal); `signer` is WHOEVER actually signed
/// this transaction (the session key, when one is used, or `player`'s own wallet directly when
/// none is). `player` deliberately can no longer be `Signer<'info>` — requiring the identity
/// itself to sign would defeat the entire point of a session key. It is still safe unsigned:
/// `#[session(authority = player.key())]` below makes `player` a seed the real SessionToken PDA's
/// address must already match (see `SessionToken::validate` in the session-keys crate), so a
/// caller can't just name an arbitrary wallet — only a token that wallet's own signature actually
/// created will resolve.
