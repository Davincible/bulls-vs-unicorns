 ...
 *  conservation PASS on a round whose fee it never actually read — the failure mode is a verification
 *  that verifies nothing, which is worse than not running. This script's whole job is to check the
 *  fee arithmetic; if the fee is not there, there is nothing here to check. */
function requireFeeField(raw: RawRoundAccount): void {
  if (raw.feesCollected === undefined) {
    throw new Error(
      ...
      "verifies the entry fee and cannot do so against that program. Deploy the current lib.rs and " +
