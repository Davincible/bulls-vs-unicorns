...
describe("advanceReplay", () => {
  ...
  it("announces every event at the rate the chain actually produces them", () => {
    // The guard against over-correcting. A throttle that fired in normal play would silently delete
    // the flourish this whole pass exists to add, so the threshold is checked against the real pace:
    // sixteen fighters is `stepsPerSecond = 32`, i.e. about 0.53 steps per 60Hz frame, and roughly
    // half of those produce an exchange.
    const state = createReplay(fighters(16));
    const events = stream(400);
    let announced = 0;
    let crossed = 0;
    for (let frame = 1; frame <= 800; frame++) {
      advanceReplay(state, events, (frame * 32) / 60, (_event, announce) => {
        crossed++;
